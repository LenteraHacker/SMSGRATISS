...
if [[ -z $(command -v curl) ]]; then
  printf "pkg install curl -y ketik itu dan enter \n"
####################################
elif [[ -z $(command -v grep) ]]; then
  printf "pkg install grep -y ketik itu dan enter \n"
elif [[ -z $(command -v tput) ]]; then
  printf "pkg install tput -y ketik dan enter"
fi
}
ngchecking
...
function negetgan(){
  getcookie=$(curl -s 'http://sms.payuterus.biz/alpha/index.php?a=keluar' -I \
         ...
         if [[ $okeey =~ "SMS Gratis Telah Dikirim" ]]; then
             echo -e "${GREEN}[+] ${NORMAL}mantap berhasil kirim st"
         else
             echo -e "${RED}[+] ${NORMAL}gagal kirim  st"
         fi
}
nebanner
echo -en "[+] NOMOR :"; read now
echo -en "[+] PESAN :"; read psn
(
for x in okeey; do
